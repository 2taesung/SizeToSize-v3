...
def get_review_data(request):
    ...
    reviewer_hash = {}
    brand = ['나이키', '아디다스', '리복', '컨버스', '퓨마', '아식스', '반스', '뉴발란스']
    search_pages = 5 # 5
    for b in brand:
        for i in range(1, search_pages+1):
            driver.get(f'https://search.musinsa.com/search/musinsa/goods?q={b}+운동화&list_kind=small&sortCode=pop&sub_sort=&page={i}&display_cnt=0&saleGoods=&includeSoldOut=&popular=&category1DepthCode=&category2DepthCodes=&category3DepthCodes=&selectedFilters=&category1DepthName=&category2DepthName=&brandIds=&price=&colorCodes=&contentType=&styleTypes=&includeKeywords=&excludeKeywords=&originalYn=N&tags=&saleCampaign=false&serviceType=&eventType=&type=&season=&measure=&openFilterLayout=N&selectedOrderMeasure=&shoeSizeOption=&d_cat_cd=')
                    ...
                    for i in range(1, review_pages+1):
                        ...
                        for i in range(1, reviews_amount+1):
                            ...
                            try:
                                ...
                                ShoesDataset.objects.create(reviewer_pk=reviewer, foot_size=int(expected_foot_size_elem), brand=b, model_name=str(model_elem), shoe_size=int(size_elem), size_standard=size_standard, size_ratio=float(size_ratio))
