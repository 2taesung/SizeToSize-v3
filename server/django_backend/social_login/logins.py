...
class SocialLogin:
    ...
    def __init__(self):
        if request.headers['Platform'].lower() not in self.platforms:
            return JsonResponse({'message': 'LOGIN KEY ERROR'}, status=400)
        
        if request.headers['Platform'] == 'kakao':
            return self.kakao_login
        
        # if request.headers['Login'] == 'naver':
